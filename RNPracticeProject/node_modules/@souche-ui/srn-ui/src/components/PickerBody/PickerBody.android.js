 ...
 * */
export default class PickerBody extends Component{
    constructor(props){
        super(props);
    ...
    shouldComponentUpdate(nextProps){
        ...
        this._scrollView && this._scrollView.scrollTo({x: 0, y: this._findNearEnableIndex(this.selectedIndex) * this.itemHeight, animated: false});
        return false;
    }
    ...
    _renderItems(items){
        ...
        for (let i = 0; i < Math.floor(this.showItemsNum / 2); i++){
            itemView.unshift(<Text key={-(i + 1)} style={[styles.itemText, itemStyle]}/>);
            itemView.push(<Text key={this.items.length + i + 1} style={[styles.itemText, itemStyle]}/>);
        }
        ...
        return (
            <View style={containerStyle}>
                ...
                <View pointerEvents={'none'} style={selectedValueContainerStyle}>
                    <ScrollView
                        ref={c => { this._selectedScrollView = c; }}
                        showsVerticalScrollIndicator={false}
                        overScrollMode={'never'}
                        scrollEnabled={false}
                    >
    ...
    componentDidMount(){
        setTimeout(()=>{this._scrollView && this._scrollView.scrollTo({x: 0, y: this._findNearEnableIndex(this.selectedIndex) * this.itemHeight, animated: false});}, 0);
    }
    componentDidUpdate(){
        setTimeout(()=>{this._scrollView && this._scrollView.scrollTo({x: 0, y: this._findNearEnableIndex(this.selectedIndex) * this.itemHeight, animated: false});}, 0);
    }
    ...
    _onScroll = (e) => {
        this._selectedScrollView.scrollTo({x: 0, y: e.nativeEvent.contentOffset.y, animated: false});
    };
    ...
    _adjustScrollDy(dy){
        ...
        const timesDy = getNearTimesNum(dy, this.itemHeight),
            selectedIndex = timesDy / this.itemHeight,
            enableSelectedIndex = this._findNearEnableIndex(selectedIndex);
        this._scrollView && this._scrollView.scrollTo({x: 0, y: enableSelectedIndex * this.itemHeight, animated: true});
 ...
 * */
function getNearTimesNum(num, unit) {
    let result,
        times = num / unit;
    if (num > 0){
...
const styles = StyleSheet.create({
    indicator: {
        alignSelf: 'stretch',
        borderColor: theme('color_text_caption'),
